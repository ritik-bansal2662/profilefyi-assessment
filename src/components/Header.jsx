            ...
            <div>
                <ul className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static bg-white md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${open ? 'top-12' : 'top-[-490px]'}`}>
                    <li className='font-semifold my-7 md:my-0 md:ml-8'>
                        <Link to="/profilefyi-assessment" className='text-3xl' onClick={() => setOpen(false)}>
                            <AiFillHome />
                        </Link>
                    </li>
                    <li className='font-semifold my-7 md:my-0 md:ml-8'>
                        <Link className='relative text-3xl' to="/profilefyi-assessment/cart" onClick={() => setOpen(false)}>
                            <FaCartShopping />
                            {
                    ...
                    <li className='font-semifold my-7 md:my-0 md:ml-8'><Link to="/profilefyi-assessment/contact-us" className='text-xl' onClick={() => setOpen(false)}>Contact Us</Link></li>
                    {
                        // links.map( (link, index) => <li key={index} className='font-semifold my-7 md:my-0 md:ml-8'><Link to={link.path}>{link.name}</Link></li>)
                    }
                    <Link to='/profilefyi-assessment/login' onClick={() => setOpen(false)} className='text-xl btn bg-blue-600 text-white md:ml-8 font-semibold px-3 py-1 rounded duration-500 md:static'>Login</Link>
                </ul>
            </div>
        </div>
    </div>
